...
Keep the SafeNet measurement UI as the presentation layer and use same-origin LibreSpeed-compatible timed multi-stream transfers for the data plane. The upstream project’s browser source is not reliably available through the workspace npm registry, so avoid making the app depend on an unresolvable package.
...
**How to apply:** Preserve the existing result shape and controls, keep the transport endpoints same-origin, and treat download/upload/ping endpoint behavior as a compatibility contract.