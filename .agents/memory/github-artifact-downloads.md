...
When a normal Git remote still rejects its credential after GitHub write access is restored, the configured `GITHUB_RELEASE_TOKEN` can be passed only through a temporary Git credential helper (or `GH_TOKEN` for `gh`), without printing it. This can publish an exact branch or tag while the GitHub connector remains read-only for low-level Git object writes. Large JSON blob uploads through the connector can also be truncated, so prefer native Git transport for large files.
...
**How to apply:** Prefer the GitHub connector for metadata and the GitHub CLI for archive downloads. If a maintainer-authorized release push is required, use the workspace-provided release token only as an in-process credential, verify the remote branch or peeled tag SHA, and never expose the token in logs or chat.
