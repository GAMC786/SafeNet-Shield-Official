...
test("Measure Your Network keeps ISP profiling and uses Cloudflare's browser engine", () => {
  ...
  assert.match(routesSource, /api\/speedtest\/upload/);
...
test("Settings use the current package version and expose only current controls", () => {
  ...
  assert.match(firewallSource, /SafeNet firewall rules are not currently enforced on the Windscribe tunnel/);
  assert.match(firewallSource, /"Android only"/);
  assert.match(firewallSource, /"Checking"/);
  assert.match(firewallSource, /"Disconnected"/);
  assert.match(firewallSource, /"Profile required"/);
