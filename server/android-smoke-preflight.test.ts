...
test("main-branch APK-only workflow builds and uploads a signed APK and checksum", () => {
  ...
  assert.match(
    apkOnlyWorkflow,
    /aapt.*dump badging "\$apk" \| grep -F "package: name='com\.safenet\.dns' versionCode='16' versionName='\$APP_VERSION'"/,
  );
...
test("tagged releases use the hosted emulator with reduced validation", () => {
  ...
  assert.match(
    releaseVerifyStep,
    /versionCode='16' versionName='\$expected_version'/,
  );
  assert.match(
    releaseVerifyStep,
    /package: name='com\.safenet\.dns\.test' versionCode='16' versionName='\$expected_version'/,
  );
