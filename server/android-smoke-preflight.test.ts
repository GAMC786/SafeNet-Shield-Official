...
test("main-branch APK-only workflow builds and uploads a signed APK and checksum", () => {
  ...
  assert.match(
    apkOnlyWorkflow,
     /aapt.*dump badging "\$apk" \| grep -F "package: name='com\.safenet\.dns' versionCode='34' versionName='\$APP_VERSION'"/,
  );
...
test("tagged releases use the hosted emulator with reduced validation", () => {
  ...
  assert.match(
    releaseVerifyStep,
    /versionCode='34' versionName='\$expected_version'/,
  );
  assert.match(
    releaseVerifyStep,
...
test("release APK verification validates app and instrumentation badging independently", () => {
  ...
  const result = runReleaseApkVerificationFixture({
    appBadging:
       "package: name='com.safenet.dns' versionCode='34' versionName='1.0.42'",
    testBadging: [
