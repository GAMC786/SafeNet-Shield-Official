...
write_blocked_result() {
        ...
        { echo "ERROR: invalid blocker category '$category'." >&2; exit 2; }
    {
        ...
        printf 'headplane_node_name=%s\n' "${headplane_node_name:-NOT_RECORDED}"
...

{
    ...
    printf 'headplane_node_name=%s\nheadplane_node_visibility=%s\nheadplane_node_status=%s\nheadplane_node_owner=%s\n' \
        "$headplane_node_name" "$headplane_node_visibility" "$headplane_node_status" "$headplane_node_owner"
