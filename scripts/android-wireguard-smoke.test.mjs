...
test("Android smoke runs the configured WireGuard release check", () => {
  ...
  assert.match(instrumentation, /WIREGUARD_TUNNEL result=PASS state=UP transport=VPN route=PASS/);
  assert.match(instrumentation, /WIREGUARD_HANDSHAKE result=PASS gateway_dns=PASS/);
  assert.match(instrumentation, /WIREGUARD_DNS result=PASS response=VALID/);
  assert.match(instrumentation, /WIREGUARD_HTTPS result=PASS ordinary_https=PASS/);
...
test("physical validation records resolver modes, internet reachability, and dashboard handoff", () => {
  ...
  assert.match(physicalConnectivityScript, /PHYSICAL_WIREGUARD_SWITCH result=PASS/);
  ...
  assert.match(physicalConnectivityScript, /wireguard_handshake_status="NOT_RECORDED"/);
  assert.match(physicalConnectivityScript, /wireguard_tunnel_status="NOT_RECORDED"/);
  assert.match(physicalConnectivityScript, /wireguard-evidence\.txt/);
  ...
  assert.match(uiInstrumentation, /dashboardWireGuardSwitchControlsTheSingleTunnel/);
  assert.match(uiInstrumentation, /off_to_on=PASS on_to_off=PASS/);
});